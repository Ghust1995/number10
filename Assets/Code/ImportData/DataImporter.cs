...
{
    private static List<Type> GetTypesWith<TAttribute>(bool inherit = true)
                              where TAttribute : System.Attribute
    {
        var assembly = Assembly.GetExecutingAssembly();
        var types = assembly.GetTypes().Where((t) => t.IsDefined(typeof(TAttribute), inherit));
        return types.ToList();
    }

    void Awake()
    {
        var dataToImport = GetTypesWith<ImportData>();
    }


    static string[] importNames = new string[]  { "abilities" };

	static DataImporter instance;
